...
DNT_PillbugAtk1 = Skill:new{
	...
	ImpSound = "/impact/generic/general",--"/props/boulder_impact",
	Range = 5,
	Damage = 1,
	Crack = false,
	TipImage = {
...
SkillEffect["DNT_AddQueuedBurst"] = function(self, ...) -- add our own queued burst
	local fx = SkillEffect()
	fx["AddBurst"](fx, ...)
	self.q_effect:AppendAll(fx.effect)
end
...
function DNT_PillbugAtk1:GetSkillEffect(p1, p2)
	...
	if Board:IsBlocked(p2,PATH_PROJECTILE) then
		local tempDam = SpaceDamage(p2,self.Damage)
		tempDam.sSound = self.ImpSound
		ret:DNT_AddQueuedArtillery(tempDam,self.Projectile,NO_DELAY) -- jump effect
		...
		ret:DNT_AddQueuedBurst(p2,"Emitter_Burst_$tile",DIR_NONE)
		
		for i = 1, 8 do -- bounce loop
			...
			ret:AddQueuedDelay(0.78)
			tempDam = SpaceDamage(p3,self.Damage)
			tempDam.sSound = self.ImpSound
			ret:AddQueuedDamage(tempDam)
			ret:DNT_AddQueuedBurst(p3,"Emitter_Burst_$tile",DIR_NONE)
		end
	...
	if Board:GetTerrain(p3) ~= TERRAIN_WATER and Board:GetTerrain(p3) ~= TERRAIN_HOLE then
		ret:AddQueuedScript("Game:TriggerSound('/enemy/leaper_1/land')")
	end
