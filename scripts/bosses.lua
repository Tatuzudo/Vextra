...
--local oldEndMission = Mission_JunebugBoss.MissionEnd
function Mission_JunebugBoss:MissionEnd()
	if Board:IsPawnAlive(self.BossID) then
		local space = Board:GetPawn(self.BossID):GetSpace()
		local effect = SkillEffect()
		local damage = SpaceDamage(space)
		damage.bEvacuate = true
		damage.fDelay = ANIMS.DNT_junebuge.Time * ANIMS.DNT_junebuge.NumFrames
		effect:AddDamage(damage)
		local script = SpaceDamage(space)
		script.sScript = string.format("Board:GetPawn(%s):SetInvisible(true)", tostring(self.BossID))
		effect:AddDamage(script)
		Board:AddEffect(effect)
	end
	--BOTH OF THESE CAUSE STACK OVERFLOW
	--Mission.MissionEnd(self)
	--oldEndMission(self)
	--The old end mission since I can't use the easy method for some reason.
	...
	for i = 0, board_size.x - 1 do
		for j = 0, board_size.y - 1  do
			if Board:IsPawnTeam(Point(i,j),TEAM_ENEMY) and Board:GetPawn(Point(i,j)):GetId() ~= self.BossID then --Now I'm just embracing the copy and paste, this causes the corpse to show up too soon
